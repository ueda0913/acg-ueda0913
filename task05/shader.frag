...
float SDF(vec3 pos)
{
  ...
  float b0 = sdBox(pos, vec3(box_size, box_size, box_size));
  float s0 = sdSphere(pos, rad_sphere);
  return max(-min(min(d0, d1), d2), max(b0, s0));
}
...
vec3 SDF_color(vec3 pos)
{
  ...
  if (max(b0, s0) < 0.){
    ...
    return vec3(0., 0., 1.); // comment out and define new color
  }
}
