  ...
  for (unsigned int iw = 0; iw < img_width; ++iw) {
    for (unsigned int ih = 0; ih < img_height; ++ih) {
      ...
      for (int isample = 0; isample < nsample; ++isample) {
        ...
        float rad = hit1_rad * hit0_brdf * hit0_normal.dot(hit0_refl) / hit0_pdf / nsample;
      ...
      for (int isample = 0; isample < num_half_sample; ++isample) {
        ...
        float rad = hit1_rad * hit0_brdf * hit0_normal.dot(hit0_refl) / (hit0_pdf_brdf_sample + hit0_pdf_light_sample) / num_half_sample;
