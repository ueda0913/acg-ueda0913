  ...
  for (unsigned int ih = 0; ih < height; ++ih)
  {
    for (unsigned int iw = 0; iw < width; ++iw)
    {
      ...
      for (unsigned int iedge = 0; iedge < num_vtx; ++iedge)
      {
        ...
        float norm_mul = sqrtf(powf(p0x, 2.0) + powf(p0y, 2.0)) * sqrtf(powf(p1x, 2.0) + powf(p1y, 2.0));
        float cos_ang = (p0x * p1x + p0y * p1y) / norm_mul;
        float sin_ang = (p0x * p1y - p0y * p1x) / norm_mul;
        winding_number += atan2(sin_ang, cos_ang) / 2 / M_PI;
      }
      int int_winding_number = int(std::round(winding_number)) % 2;
      if (int_winding_number < 0)
      {
        int_winding_number += 2;
      }
      // printf("%d\n", int_winding_number);
    ...
    unsigned char brightness)
{
  ...
  if (std::abs(dy) <= std::abs(dx))
  {
    ...
    for (int i = 0; i < int(x1) - int(x0) + 1; ++i)
    {
      const int y_index = int(y0 + m * (i));
      const int x_index = int(x0) + i;
      img_data[width * y_index + x_index] = brightness;
    }
  }
  else
  {
    ...
    for (int i = 0; i < int(y1) - int(y0) + 1; ++i)
    {
      const int x_index = int(x0 + m * (i));
      const int y_index = int(y0) + i - 1;
      img_data[width * y_index + x_index] = brightness;
    }
  }
}
